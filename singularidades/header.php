<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN"
   "http://www.w3.org/TR/html4/strict.dtd">
<html>

<head>
    <title>Instituto Singularidades</title>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"> 
    <meta name="description" content="">
    <meta name="keywords" lang="pt-br" content="">
    <meta name="Author" content="Fess'Kobbi">
    <link href="favicon.png" rel="icon" type="image/x-icon" />
    <link rel="stylesheet" href="css/main.css">
</head>

<body>
<!--Container-->
<div id="container">
    <!--Header-->
    <div id="header">
        <div class="content-center">
            <!--Logo-->
            <a href="/" title="Voltar para a página inicial" id="logo" class="link-padding main-icons">Instituto Singularidades - Formando professores, transformando o futuro</a>
            <!--End of Logo-->
            
            <!--Header contents-->
            <div class="header-contents">
                <!--Search form-->
                <form>
                    <input type="text" name="main-menu-search" id="main-menu-search" placeholder="Pesquisa">
                    <button type="button" name="main-menu-search-button" id="main-menu-search-button" class="link-padding main-icons">Search</button>
                    <div class="clear"></div>
                </form>
                <!--End of Search form-->
                
                <div class="portais-link">
                    <a href="#" class="portal-aluno main-icons">Portal do aluno</a>
                    <a href="#" class="portal-professor main-icons">Portal do professor</a>
                    <div class="clear"></div>
                </div>
                <div class="clear"></div>
                
                <!--Main menu #1-->
                <ul id="main-menu-1" class="main-icons">
                    <li><a href="#" title="Como ingressar">Como ingressar</a></li>
                    <li><a href="#" title="Serviços ao aluno">Serviços ao aluno</a></li>
                    <li><a href="#" title="Revista eletrônica">Revista eletrônica</a></li>
                    <li><a href="#" title="Programa de bolsas">Programa de bolsas</a></li>
                    <li><a href="#" title="Biblioteca">Biblioteca</a></li>
                    <li><a href="#" title="Escolas parceiras">Escolas parceiras</a></li>
                    <li><a href="#" title="Contato">Contato</a></li>
                </ul>
                <!--End of Main menu #1-->
                <div class="clear"></div>
            </div>
            <!--End of Header contents-->
            <div class="clear"></div>
        </div>
        
        <!--Main Menu #2-->
        <div class="main-menu-2">
            <div class="content-center">
                <ul>
                    <li><a href="#" title="O Instituto">O Instituto</a></li>
                    <li><a href="#" title="Graduação">Graduação</a></li>
                    <li><a href="#" title="Pós-graduação">Pós-graduação</a></li>
                    <li><a href="#" title="Extensão">Extensão</a></li>
                    <li><a href="#" title="EAD">EAD</a></li>
                </ul>
                <div class="clear"></div>
            </div>
        </div>
        <!--End of Main Menu #2-->
    </div>
    <!--End of Header-->
    
    <!--Content-->
    <div id="content">
        <div class="content-center">